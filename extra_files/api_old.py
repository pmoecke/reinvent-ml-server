...
app = FastAPI(title="SceneScript API", description="API for 3D scene description using SceneScript model")
...
async def upload_files(
    ...
    try:
        ...
        if semidense_points:
            points_filename = f"uploaded_semidense_points_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv.gz"
        ...
        if trajectory:
            traj_filename = f"uploaded_trajectory_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        for f in os.listdir(uploads_dir):
            ...
            if os.path.isfile(file_path):
                files.append({
                    ...
                    "modified": pd.Timestamp.fromtimestamp(os.path.getmtime(file_path)).isoformat()
                })
